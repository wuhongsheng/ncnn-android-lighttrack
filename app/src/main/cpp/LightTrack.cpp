...
LightTrack::LightTrack(AAssetManager* mgr) {
    score_size = int(round(this->instance_size / this->total_stride));
    this->load_model(mgr);
...
void LightTrack::init(const uint8_t *img, Bbox &box, int im_h , int im_w) {
    ori_img_h = im_h;
    ori_img_w = im_w;
    LOGI("ori_img_w: %d ori_img_h: %d", ori_img_w, ori_img_h);
    ...
    LOGI("target_sz_x: %f target_sz_y: %f", target_sz.x, target_sz.y);

    LOGI("box.x0: %f box.y0: %f box.x1: %f box.y1: %f", box.x0, box.y0,box.x1,box.y1);
...
//        LOGI("use_vulkan_compute %d",ncnn::get_gpu_count());
//    }
    ...
    LOGI("target_pos_x: %d target_pos_y: %d", this->target_pos.x, this->target_pos.y);
...
//    LOGI("pscore_window max score is: ",pscore[r_max * cols + c_max]);
...
void LightTrack::track(const uint8_t *img) {
    ...
    LOGI("track target pos: %d %d", target_pos.x, target_pos.y);
    LOGI("track target_sz: %f %f", target_sz.x, target_sz.y);
...
void LightTrack::load_model(AAssetManager* mgr) {
    this->net_init.load_param(mgr,"lighttrack_init.param");
    this->net_init.load_model(mgr,"lighttrack_init.bin");
    this->net_update.load_param(mgr,"lighttrack_update.param");
    this->net_update.load_model(mgr,"lighttrack_update.bin");
}
